...
class MonsterAction(BaseModel):
    ...
    target_character_id: Optional[str] = Field(default=None, description="스킬을 사용할 대상의 ID")
...
class BattleActionResponse(BaseModel):
    # monster_id: str = Field(description="현재 몬스터의 ID")
    current_character_id: str = Field(description="현재 몬스터의 ID")
...
class CharacterForAI(CharacterConfig, CharacterState):
    distance: int = Field(description="현재 몬스터와 캐릭터 사이의 거리")
